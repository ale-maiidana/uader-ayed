...
class BinaryTree:
    ...
    def proximity_search(self, value: Any) -> list:
        aux = []

        def __search(root, value):
            if root is not None:
            # Convertimos a string para evitar errores con int, float, etc.
                if str(root.value).startswith(str(value)):
                    aux.append(root)
    ...
    def by_level(self):
        tree_queue = Queue() 
        if self.root is not None: tree_queue.arrive(self.root)
